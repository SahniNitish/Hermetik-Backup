...
async function testUser12API() {
  try {
    console.log('🔍 Testing user12 wallet API...\n');
    ...
    console.log('Response structure:', {
      ...
      portfolios: walletResponse.data.data.portfolios.length,
    ...
    walletResponse.data.data.portfolios.forEach((portfolio, index) => {
      ...
      if (portfolio.protocols && portfolio.protocols.length > 0) {
        ...
        portfolio.protocols.forEach((protocol, pIndex) => {
      ...
      if (portfolio.tokens && portfolio.tokens.length > 0) {
        ...
        portfolio.tokens.slice(0, 3).forEach((token, tIndex) => {
